...
module.exports = function (app, passport, connection) {
        ...
        connection.query("SELECT * FROM asesores WHERE username = ?", [req.body.username], function (err, rows) {
            ...
            if (rows.length) {
                ...
                if (typeof (req.file) == 'undefined') {
                    ...
                    connection.query(insertQuery, [req.body.nombre, req.body.telefono, req.body.username, bcrypt.hashSync(req.body.password, null, null), req.body.socio, req.file.location, req.body.color], function (err, rows) {
                        res.redirect('/panel?agregado=1');
                    });
                }
            }
    ...
    app.post('/editarasesor', inicioSesion, upload.single('image'), function (req, res) {
        if (typeof (req.file) != 'undefined') {
            /*Cambio su foto*/
            connection.query('UPDATE asesores SET nombre = ?, telefono = ?, username = ?, password = ?, foto = ?, color=? WHERE idasesores = ? ', [req.body.nombre, req.body.telefono, req.body.username, bcrypt.hashSync(req.body.password, null, null), req.file.location, req.body.color, req.user.idasesores], function (err, result) {
    ...
    app.get('/rss', function (req, res) {
        ...
        var feed = new Feed({
            ...
            link: 'http://' + variables.dominio + '/',
            image: 'http://' + variables.dominio + '/logoup.png',
            copyright: 'Copyright © 2016 ' + variables.nombreinmobiliaria + '. All rights reserved',
            author: {
                name: variables.nombreinmobiliaria,
            }
        });
        ...
        connection.query('SELECT * FROM propiedades', function (err, propiedades) {
            propiedades.forEach(function (propiedad) {
                feed.item({
                    ...
                    link: 'http://' + variables.dominio + '/propiedad/?id=' + propiedad.idpropiedades,
